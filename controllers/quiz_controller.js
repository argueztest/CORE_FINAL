...
if(!search){
	...
	var search_aux="%"+search.replace(" ","%")+"%";

models.Quiz.findAll({where: {question: {$like: search_aux}},include: [ models.Attachment] })
		.then(function(quizzes) {
			res.render('quizzes/index.ejs', { quizzes: quizzes,search :search});
		})
		.catch(function(error) {
...
*/
exports.new = function(req, res, next){
  var redir = req.query.redir || "/quizzes";
  var quiz = models.Quiz.build({question: "", answer: ""});
  res.render('quizzes/new', {quiz: quiz, redir: redir});
};
// POST /quizzes/create
/*
...
// FUNCIONES AUXILIARES para subir imagenes
...
function createAttachment(req, uploadResult, quiz) {
    if (!uploadResult) {
        ...
        console.log("createAttachment: NO SE SUBIO");
    }
